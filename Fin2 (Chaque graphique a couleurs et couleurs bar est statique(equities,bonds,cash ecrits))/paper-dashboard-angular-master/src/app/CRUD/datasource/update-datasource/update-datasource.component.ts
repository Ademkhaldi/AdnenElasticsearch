...
export class UpdateDatasourceComponent implements OnInit {
  ...
  updateDatasource() {
    ...
    if (this.datasource.connection_port.toString().length > 4) {
      this.errorMessages.push({ inputId: 'connection_port', message: "connection_port ne peut pas dépasser 4 caractères" });
    }
  
    if (this.datasource.connection_port === 0) {
      this.errorMessages.push({ inputId: 'connection_port', message: "connection_port ne peut pas être égal à 0" });
    }
  
    if (!this.datasource.url.trim()) {
      this.errorMessages.push({ inputId: 'url', message: "L'URL ne peut pas être vide." });
    }
    ...
    if (this.datasource.password.length === 0) {
      this.errorMessages.push({ inputId: 'password', message: "password ne peut pas être vide" });
    }
    ...
    this.datasourceService.getElasticsearchUser().subscribe((elasticUser: string) => {
      ...
      this.datasourceService.getElasticsearchPort().subscribe((elasticPort: number) => {
        if (this.datasource.connection_port != elasticPort) {
          this.errorMessages.push({ inputId: 'connection_port', message: "Faux Port Elasticsearch" });
        }
